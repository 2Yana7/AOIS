...
"""из десятичного формата в двоичный в прямом  код"""
def decimal_to_binary(n: int, bits=INT_BITS) -> str:
    if n >= 0:
...
def decimal_to_ones_complement(n: int, bits=INT_BITS) -> str:
    if n >= 0:
        return int_to_bin(n, bits)
    else:
...
def decimal_to_twos_complement(n: int, bits=INT_BITS) -> str:
    if n >= 0:
        return int_to_bin(n, bits)
    else:
        value = (1 << bits) - abs(n)
...
def binary_addition(a: int, b: int, bits=INT_BITS) -> int:
    max_val = 1 << bits
    result = (a + b) % max_val
    if result >= (1 << (bits - 1)):
        result -= (1 << bits)
...
def binary_subtraction(a: int, b: int, bits=INT_BITS) -> int:
    return binary_addition(a, -b, bits)

"""Умножает два числа"""
def binary_multiplication(a_bin: str, b_bin: str, bits=INT_BITS) -> str:
    sign = "1" if a_bin[0] != b_bin[0] else "0"
    a_abs = bin_to_int(a_bin[1:])
    b_abs = bin_to_int(b_bin[1:])
    product = a_abs * b_abs
    max_val = 1 << (bits - 1)
    product = product % max_val
...
def main():
    ...
    if choice == '2':
        ...
        product_bin = binary_multiplication(a_bin, b_bin, bits)
